...
<#macro formSingleSelectExt path options  attributes="">
    <@bind path/>
<select id="${status.expression?replace('[','')?replace(']','')}" name="${status.expression}" ${attributes}>
    <#list options as value>
        <option value="${value.key?html}"<@checkSelected value.key/>>${value.value?html}</option>
    </#list>
...
<#--
*是否具有某个权限 arg为数字是比较id，其他比较name
-->
<#function  hasPermission arg>
    <#if http_subject??>
        <#list http_subject.permissions as value>
            <#local tp=arg?is_number/>
            <#if tp>
                <#if arg=value.id>
                    <#return true>
                </#if>
