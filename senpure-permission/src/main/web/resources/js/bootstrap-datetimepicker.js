 ...
 * ========================================================= Copyright 2012
 * Stefan Petre Improvements by Andrew Rowls Improvements by Sébastien Malot
 * Improvements by Yun Lai Improvements by Kenneth Henderick Project URL :
...
!function($) {
	...
	function UTCToday() {
		...
		return UTCDate(today.getUTCFullYear(), today.getUTCMonth(), today
						.getUTCDate(), today.getUTCHours(), today
	...
	var Datetimepicker = function(element, options) {
		...
		this.component = this.element.is('.date')
				? (this.bootcssVer == 3
						? this.element
								.find('.input-group-addon .glyphicon-th, .input-group-addon .glyphicon-time, .input-group-addon .glyphicon-calendar, .input-group-addon .glyphicon-calendar, .input-group-addon .fa-calendar, .input-group-addon .fa-clock-o')
								.parent()
						: this.element
								.find('.add-on .icon-th, .add-on .icon-time, .add-on .icon-calendar .fa-calendar .fa-clock-o')
								.parent())
				: false;
		this.componentReset = this.element.is('.date')
				? (this.bootcssVer == 3
						? this.element
								.find(".input-group-addon .glyphicon-remove, .input-group-addon .fa-times")
								.parent()
						: this.element
		...
		var template = this.bootcssVer === 3
				? DPGlobal.templateV3
				: DPGlobal.template;
		...
		this.picker = $(template).appendTo(this.isInline
				? this.element
				: this.container) // 'body')
				.on({
		...
		if (this.isRTL) {
			...
			var selector = this.bootcssVer === 3
					? '.prev span, .next span'
					: '.prev i, .next i';
			this.picker.find(selector).toggleClass(this.icons.leftArrow + ' '
	...
	Datetimepicker.prototype = {
		...
		setValue : function() {
			...
			if (this.linkField) {
				$('#' + this.linkField).val(this
						.getFormattedDate(this.linkFormat));
			}
		},
		...
		fill : function() {
			...
			var d = new Date(this.viewDate), year = d.getUTCFullYear(), month = d
					...
					.getUTCHours(), minutes = d.getUTCMinutes(), startYear = this.startDate !== -Infinity
					? this.startDate.getUTCFullYear()
					: -Infinity, startMonth = this.startDate !== -Infinity
					? this.startDate.getUTCMonth() + 1
					: -Infinity, endYear = this.endDate !== Infinity
					? this.endDate.getUTCFullYear()
					: Infinity, endMonth = this.endDate !== Infinity
			...
			this.picker.find('.datetimepicker-days tbody').empty().append(html
					.join(''));
					...
					case 'span' :
						if (!target.is('.disabled')) {
							...
							if (target.is('.month')) {
								...
								minutes = parseInt(target.text().substr(target
												.text().indexOf(':')
												+ 1), 10)
										|| 0;
		...
		showMode : function(dir) {
			if (dir) {
				var newViewMode = Math.max(0, Math.min(DPGlobal.modes.length
										- 1, this.viewMode + dir));
			...
			this.picker.find('>div').hide().filter('.datetimepicker-'
	...
	var DPGlobal = {
		...
		parseFormat : function(format, type) {
			...
			var separators = format.replace(this.validParts(type), '\0')
					.split('\0'), parts = format.match(this.validParts(type));
		...
		formatDate : function(date, format, language, type) {
			var description = "";
			for (var i in format) {
				var property = format[i];
				description += i + " = " + property + "\n";
			}
			console.log(description);
			console.log('date' + date + ',format' + format + ',language'
					+ language + ',type' + type)
			if (date == null) {
				return '';
			}
			...
			if (type == 'standard') {
				val = {
					...
					p : (dates[language].meridiem.length == 2
							? dates[language].meridiem[date.getUTCHours() < 12
									? 0
									: 1]
							: ''),
					// hour
				...
				val = {
					...
					a : (dates[language].meridiem.length == 2
							? dates[language].meridiem[date.getUTCHours() < 12
									...
									: 1]
							: ''),
					g : (date.getUTCHours() % 12 == 0 ? 12 : date.getUTCHours()
							% 12),
					G : date.getUTCHours(),
					// minute
					i : date.getUTCMinutes(),
		...
		headTemplate : '<thead>' + '<tr>'
				...
				+ '<th class="next"><i class="{rightArrow}"/></th>' + '</tr>'
				+ '</thead>',
	...
	DPGlobal.template = '<div class="datetimepicker">'
			...
			+ '</div>' + '<div class="datetimepicker-hours">'
			+ '<table class=" table-condensed">' + DPGlobal.headTemplate
			+ DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>'
			+ '</div>' + '<div class="datetimepicker-days">'
			+ '<table class=" table-condensed">' + DPGlobal.headTemplate
			+ '<tbody></tbody>' + DPGlobal.footTemplate + '</table>' + '</div>'
			+ '<div class="datetimepicker-months">'
			+ '<table class="table-condensed">' + DPGlobal.headTemplate
			+ DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>'
			+ '</div>' + '<div class="datetimepicker-years">'
			+ '<table class="table-condensed">' + DPGlobal.headTemplate
			+ DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>'
			+ '</div>' + '</div>';
	DPGlobal.templateV3 = '<div class="datetimepicker">'
			+ '<div class="datetimepicker-minutes">'
			+ '<table class=" table-condensed">' + DPGlobal.headTemplateV3
			+ DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>'
			+ '</div>' + '<div class="datetimepicker-hours">'
			+ '<table class=" table-condensed">' + DPGlobal.headTemplateV3
			+ DPGlobal.contTemplate + DPGlobal.footTemplate + '</table>'
